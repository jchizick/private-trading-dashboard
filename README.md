...
Provider keys and `DASHBOARD_PASSWORD` must stay server-side. Client components call only internal routes such as `/api/fear-greed` and `/api/market-quotes`; they must not call external providers or read provider keys directly.

`DASHBOARD_PASSWORD` enables the app-level password gate. Anonymous visitors are redirected to `/login`; successful login sets an httpOnly `trading_dashboard_auth` cookie.
...
Vercel is the assumed deployment target. Add `CMC_API_KEY`, `FMP_API_KEY`, `TWELVE_DATA_API_KEY`, and `DASHBOARD_PASSWORD` in Vercel Project Settings for the environments you use, including Production and Preview. Redeploy after changing env vars.
...
Before exposing the app on the public internet, keep `DASHBOARD_PASSWORD` configured and rotate it when needed.
